...
const TourDetail = () => {
  ...
  if (isError)
    return (
      <p className="text-center text-red-500">Failed to load tour details.</p>
    );

  const coverImage =
    data?.tour?.coverImage || "https://via.placeholder.com/600";
  const images = data?.tour?.galleryImages || [];
  const tour = data?.tour;
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <p className="text-xs sm:text-sm md:text-md lg:text-lg text-gray-700 text-center mt-6 font-bold font-montserrat tracking-wide [word-spacing:6px]">
        {tour.description || "No description available."}
      </p>

      <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl text-center font-bold text-gray-800 mt-6 font-montserrat">
        Photo Gallery
      </h2>
        ...
        {images.length > 0 ? (
          images.map((img, index) => (
            <img
              ...
              src={img}
              alt="Tour"
              className="w-full h-48 object-cover rounded-md border border-black shadow-sm"
            />
          ))
        ) : (
          <p className="text-center">No images available</p>
      ...
      <div className="grid grid-cols-1 gap-3 mt-6 text-xs sm:text-sm md:text-md lg:text-lg font-bold font-montserrat">
        <p className="flex items-center gap-2 text-gray-600">
          <FaMapMarkerAlt className="text-blue-500" />{" "}
          {tour.destinations || "Unknown Location"}
        </p>
        <p className="flex items-center gap-2 text-gray-600">
          <FaCalendarAlt className="text-green-500" /> Starting Date:{" "}
          {new Date(tour.start_date).toLocaleDateString() || "N/A"}
        </p>
        <p className="flex items-center gap-2 text-gray-600">
          <FaCalendarAlt className="text-red-500" /> End Date:{" "}
          {new Date(tour.end_date).toLocaleDateString() || "N/A"}
        </p>
        <p className="flex items-center gap-2 text-gray-600">
          <FaUsers className="text-orange-500" /> Operator:{" "}
          {tour.tourOperatorId?.name || "N/A"}
        </p>
      </div>

      {notification && (
        <div
          className={`fixed right-4 bottom-4 z-50 px-4 py-2 rounded-md text-white shadow-lg flex items-center space-x-2 
                ${
